...
public class WebhookExample
{
    ...
    /// Example 1: Register a webhook
    /// </summary>
    private static async Task<int> RegisterWebhook(CCAIClient client)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"Webhook registered successfully: ID={response.Id}, URL={response.Url}, Type={response.IntegrationType}");
    ...
    private static async Task ListWebhooks(CCAIClient client)
    {
        ...
        try
        {
            ...
            {
                Console.WriteLine($"- ID={webhook.Id}, URL={webhook.Url}, Type={webhook.IntegrationType}");
            }
        }
        catch (Exception ex)
        {
    ...
    /// Example 3: Update a webhook
    /// </summary>
    private static async Task UpdateWebhook(CCAIClient client, int webhookId)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"Webhook updated successfully: ID={response.Id}, URL={response.Url}, Type={response.IntegrationType}");
        }
        catch (Exception ex)
        {
    ...
    /// Example 4: Delete a webhook
    /// </summary>
    private static async Task DeleteWebhook(CCAIClient client, int webhookId)
    {
    ...
    private static void ParseWebhookEvent(CCAIClient client)
    {
        ...
        try
        {
            ...
            var eventHash = "event-hash-abc123"; // This would come from the webhook event
            var secret = "your-webhook-secret";

            var isValid = client.Webhook.VerifySignature(signature, clientId, eventHash, secret);
